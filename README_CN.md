...
> **混合架构QQ消息插件** - 结合NapCat的稳定消息接收和AstrBot的可靠消息推送

## 🎯 概述

Hybrid QQ Messenger 是一个专为 OpenClaw 设计的插件，通过巧妙的混合架构提供稳定可靠的QQ消息处理能力：

- **NapCat集成**: 通过 OneBot v11 WebSocket 协议稳定接收QQ消息
- **AstrBot集成**: 通过 REST API 可靠地发送主动消息
- **混合架构**: 接收和发送路径完全分离，确保最高可靠性
...
- **🎭 双路径架构**: 接收和发送完全分离，避免单点故障
- **🧠 自然对话流**: 消息由 OpenClaw AI 自然处理，不强制响应
- **🔄 会话管理**: 智能会话跟踪和自动超时处理
- **⚡ 主动消息**: 通过 AstrBot API 可靠推送主动消息
- **🔧 配置驱动**: 灵活的 JSON 配置，支持运行时更新
- **🤖 技能集成**: 自动检测和创建 OpenClaw 技能
